...
accelerometer_df = 
  ...
  janitor::clean_names() %>% 
  mutate(
    weekday_vs_end = ifelse(day %in% c('Saturday', 'Sunday'), 'Weekend', 'Weekday')) %>% 
    select(day_id, week, day, weekday_vs_end, everything()) %>% 
  pivot_longer(
...
accelerometer_df
```
* The resulting data set has `r nrow(accelerometer_df)` observations of `r ncol(accelerometer_df)` variables: `r names(accelerometer_df)`.
